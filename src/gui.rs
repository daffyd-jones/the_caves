...
impl GUI {
    pub fn new() -> Self {
        ...
        let notes_opt = (vec!["".to_string(); 3], vec!["".to_string(); 3]);
        let enc_opt = (
    ...
    pub fn move_cursor(&mut self, dir: &str) {
        match dir {
            ...
            "RT" => {
                if self.cursor_pos.0 < self.cursor_bounds[self.cursor_pos.1] - 1 {
                    self.cursor_pos.0 += 1
                }
            }
            "DN" => {
                if self.cursor_pos.1 < self.cursor_bounds.len() - 1 {
                    self.cursor_pos.1 += 1
                }
            }
            _ => todo!(),
        ...
        self.terminal
            .draw(|f| {
                ...
                let load_chars = vec![
                    "═", "║", "╣", "╠", "╩", "╦", "╗", "╝", "╚", "╔", "╬", "┐", "└", "┴", "┬", "├",
                    "─", "┼", "┘", "┌", "┤", "│", "▥", "℧", "ï", "ì", "í", "ή", "ớ", "ʑ", "ẅ", "Ѫ",
                    "▩", "▨", "▧", "ṑ", "Ѧ", "Ħ", "±", "ϙ", "ዠ", "ዣ", "ቸ", "Ͳ", "ẛ", "Ṫ", "ṓ", "ϟ",
                ];
                let a = f.area();
                let inner = a.inner(Margin {
                    horizontal: 1,
                    vertical: 1,
                });
        ...
        self.terminal
            .draw(|f| {
                ...
                match self.info_mode {
                    GUIMode::Bug => {
                        draw_debug(f, gui_args, self.cursor_pos, debug, comp_str, game_chunks);
                    }
                    GUIMode::Normal => {
                        draw_normal(f, gui_args, stats, self.cursor_pos, game_chunks);
                        self.cursor_bounds = vec![100; 100];
                    }
                    GUIMode::Map => {
                        let (cur_bounds, comp_opts) = draw_map_view(
                            f,
                            self.cursor_pos,
                            game_chunks,
                            self.comp_head,
                            self.comp_list.clone(),
                        );
                        self.cursor_bounds = cur_bounds;
                        self.comp_opts = comp_opts;
                    }
                    GUIMode::Inventory => {
                        draw_inventory(f, self.cursor_pos, game_chunks, self.inventory.clone());
                    }
                    GUIMode::Notes => {
                        draw_notes(
                            f,
...
fn draw_debug(
    ...
    let mut rows = vec![
        ...
        Row::new(vec![
            ...
            Span::styled(
                gui_args.map.viewport_x.to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Row::new(vec![
            ...
            Span::styled(
                gui_args.map.viewport_y.to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Row::new(vec![
            ...
            Span::styled(
                gui_args.map.viewport_width.to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Row::new(vec![
            ...
            Span::styled(
                gui_args.map.viewport_height.to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Row::new(vec![
            ...
            Span::styled(
                (gui_args.map.viewport_y + (gui_args.map.viewport_height / 7)).to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Row::new(vec![
            ...
            Span::styled(
                ((gui_args.map.viewport_height + gui_args.map.viewport_y)
                    - (gui_args.map.viewport_height / 7))
                    .to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Row::new(vec![
            ...
            Span::styled(
                (gui_args.map.viewport_x + (gui_args.map.viewport_width / 7)).to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Row::new(vec![
            ...
            Span::styled(
                ((gui_args.map.viewport_width + gui_args.map.viewport_x)
                    - (gui_args.map.viewport_width / 7))
                    .to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        ...
        Row::new(vec![
            ...
            Span::styled(
                (gui_args.map.tunnels.len()).to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Row::new(vec![
            ...
            Span::styled(
                (gui_args.enemies.len()).to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Row::new(vec![
        ...
        Row::new(vec![
            ...
            Span::styled(
                (gui_args.puzzle_pieces.unwrap().len()).to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Row::new(vec![
        ...
        Row::new(vec![
            ...
            Span::styled(
                (gui_args.env_inter.as_ref().unwrap().len()).to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
    ];
    ...
    settle_pos[cursor_pos.1..cursor_pos.1 + 5]
        .into_iter()
        .for_each(|i| {
            rows.push(Row::new(vec![
    ...
    feat_pos[cursor_pos.1..cursor_pos.1 + 5]
        .into_iter()
        .for_each(|i| {
            rows.push(Row::new(vec![
...
fn draw_normal(
    ...
    let rows = vec![
        ...
        Row::new(vec![
            ...
            Span::styled(
                stats.player[2].to_string(),
                Style::default().fg(Color::Yellow),
            ),
            Span::styled("Luck xp: ", Style::default().fg(Color::White)),
            Span::styled(
                stats.player[3].to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ])
        .height(row_height),
        Row::new(vec![
            ...
            Span::styled(
                gui_args.player.money.to_string(),
                Style::default().fg(Color::Yellow),
            ),
            Span::styled("Herbalism xp: ", Style::default().fg(Color::White)),
            Span::styled(
                stats.player[6].to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ])
        .height(row_height),
    ...
    let column_constraints =
        vec![Constraint::Percentage(100 / buff_strs.len() as u16); buff_strs.len()];

    let buff_table = Table::new(rows, column_constraints).block(enchant_block);
...
fn draw_map_view(
    ...
    if map {
        ...
        let compass = Paragraph::new(Text::raw(content)).block(paragraph_block);
            ...
            (dx, dy) if dy > 0 && dy.abs() >= dx.abs() => {
                for y in 0..height {
                    for x in 0..width {
                        if x < cen_x
                            && y >= cen_y - 1
                            && (y + 1) as f32 >= slope * (width - (x + 3)) as f32
                        {
                            content.push('#');
                ...
                for y in 0..height {
                    for x in 0..width {
                        if x < cen_x && y < cen_y && y as f32 <= slope * x as f32 {
                            ...
                            && y < cen_y
                            && (y + 0) as f32 <= slope * (width - (x + 1)) as f32
                        {
                            content.push('#');
                        } else {
                            content.push('.');
    ...
    let xy_str = format!("target: ({}, {})", comp_head.0, comp_head.1);
    let xy_para = Paragraph::new(Text::raw(xy_str)).block(xy_block);
    ...
    let rows: Vec<Row> = inv_table
        ...
        .map(|(j, row)| {
            let cells: Vec<Cell> = row
                ...
                .enumerate()
                .map(|(i, cell)| {
                    if i == cursor_pos.0 && j == cursor_pos.1 {
...
fn draw_inventory(
    ...
    game_chunks: std::rc::Rc<[Rect]>,
    inventory: Vec<Item>,
) -> (Vec<(usize, Item)>, Vec<(usize, Item)>, Vec<(usize, Item)>) {
    let normal_info = Layout::default()
        .direction(Direction::Vertical)
    ...
    let mut inv_opt = (col1, col2, col3);
    let mut cur_bounds = Vec::new();
    ...
    let rows: Vec<Row> = (0..25)
        .map(|i| {
            let cells: Vec<Cell> = inv_table
                ...
                .enumerate()
                .map(|(j, col)| {
                    if i == cursor_pos.1 && j == cursor_pos.0 {
                        Cell::from(Span::styled(
                            col[i].1.sname.clone(),
                            ratatui::style::Style::default().fg(ratatui::style::Color::Yellow),
                        ))
    ...
    let temp = cursor_pos.0;
    let inv_option = match temp {
        0 => &mut inv_opt.0[cursor_pos.1],
        1 => &mut inv_opt.1[cursor_pos.1],
        2 => &mut inv_opt.2[cursor_pos.1],
        _ => todo!(),
    };
...
fn draw_notes(
    ...
    let rows: Vec<Row> = inv_table
        ...
        .map(|(j, row)| {
            let cells: Vec<Cell> = row
                ...
                .map(|(i, cell)| {
                    if menu_lvl > 0 {
                        if i == cursor_hold.0 && j == cursor_hold.1 {
                            Cell::from(Span::styled(
                                cell.as_str(),
                                ratatui::style::Style::default().fg(ratatui::style::Color::Yellow),
                            ))
    ...
    let c_hold = cursor_hold.0;

    let cursor_bounds = vec![4; 12];

    let paragraph = if menu_lvl == 0 {
        match cursor_pos.0 {
            0 => {
                let mut tvec = Vec::new();
                for (i, v) in active_notes.0.keys().enumerate() {
                    if cursor_pos.1 == i {
                        tvec.push(Line::from(Span::styled(
                            v,
                            Style::default().fg(Color::Yellow),
                        )));
                    } else {
            ...
            1 => {
                let mut tvec = Vec::new();
                for (i, v) in active_notes.1.keys().enumerate() {
                    if cursor_pos.1 == i {
                        tvec.push(Line::from(Span::styled(
                            v,
                            Style::default().fg(Color::Yellow),
            ...
            2 => {
                let mut tvec = Vec::new();
                for (i, v) in active_notes.2.keys().enumerate() {
                    if cursor_pos.1 == i {
                        tvec.push(Line::from(Span::styled(
                            v,
                            Style::default().fg(Color::Yellow),
            ...
            3 => {
                ...
                let paragraph = Paragraph::new(text)
                    .block(note_block)
                    .wrap(ratatui::widgets::Wrap { trim: true })
                    .scroll((cursor_pos.1.try_into().expect("oope"), 0));
                paragraph
            }
            _ => {
                let paragraph = Paragraph::new(Span::raw("Notes for the user.")).block(note_block);
                paragraph
            }
        }
    } else {
        match c_hold {
            0 => {
                let mut tvec = Vec::new();
                for (k, v) in &active_notes.0 {
                    tvec.push(Line::from(Span::raw(k)));
                    tvec.push(Line::from(Span::raw(v)));
                }
                let text = Text::from(tvec);
                let paragraph = Paragraph::new(text)
                    .block(note_block)
            ...
            2 => {
                let settle = active_notes
                    .2
                    .clone()
                    .into_values()
                    .collect::<Vec<String>>()[cursor_pos.1]
                    .clone();
...
fn draw_help(f: &mut Frame) {
    ...
    let h_block = Block::default().title("Game").borders(Borders::ALL);
    ...
    .block(Block::bordered().title("Key Bindings"));
