//settlement rs
use crate::enums::{Settle, Cells, NPCWrap, EnvInter};
use crate::shop::{Shop};
...
const church1: &str =
r#"ConvNPC ShopNPC|Null|HealthPotion
___________________________________________________________________________
...
const guild1: &str =
r#"ConvNPC CommNPC ConvNPC ShopNPC|Null|HealthPotion
...
▒▒▒▒▒▒▒▒▒▒▒▒_____________▒┐                │              ▒________________
▒▒▒▒_____________________▒l@              @│@             ▒________________
▒▒▒▒_____________________▒┘                │              ▒________________
▒▒▒▒_____________________▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒________________
▒▒▒▒_______________________________________________________________________
___________________________________________________________________________
...
fn parse_map(s_map: &str, mut cells: Vec<Vec<Cells>>, shop_type: Shops) -> (Vec<Vec<Cells>>, HashMap<(usize, usize), NPCWrap>, HashMap<(usize, usize), Item>, HashMap<(usize, usize), Item>, HashMap<(usize, usize), EnvInter>) {
...
fn place_small_parts(mut map: Vec<Vec<Cells>>, part: Vec<Vec<Cells>>, npcs: HashMap<(usize, usize), NPCWrap>, sitems: HashMap<(usize, usize), Item>, items: HashMap<(usize, usize), Item>, env_inter: HashMap<(usize, usize), EnvInter>, quad: u8) -> (Vec<Vec<Cells>>, HashMap<(usize, usize), NPCWrap>, HashMap<(usize, usize), Item>, HashMap<(usize, usize), Item>, HashMap<(usize, usize), EnvInter>) {
    let (sx, sy) = {
        match quad {
            1 => (0, 0),
            2 => (75, 0),
...
fn build_small_settle(is_cave_o: bool) -> (Vec<Vec<Cells>>, HashMap<(usize, usize), NPCWrap>, HashMap<(usize, usize), Item>, HashMap<(usize, usize), Item>, HashMap<(usize, usize), EnvInter>) {
   ...
   // let anchor = anchors.choose(&mut rng).expect("anchor parse failed");
    let (item_map, item_npcs, item_sitems, item_items, item_env_inter) = parse_map(item_shop, item_cell.clone(), Shops::Item);
    let (guild_map, guild_npcs, guild_sitems, guild_items, guild_env_inter) = parse_map(guild, guild_cell.clone(), Shops::Guild);
    let (church_map, church_npcs, church_sitems, church_items, church_env_inter) = parse_map(church, church_cell.clone(), Shops::Church);
    let (anchor_map, anchor_npcs, anchor_sitems, anchor_items, anchor_env_inter) = parse_map(anchor, anchor_cell.clone(), Shops::Null);
    ...
    //quads.retain(|&x| x != q1);
    let (q1_map, q1_npcs, q1_sitems, q1_items, q1_env_inter) = place_small_parts(cells.clone(), item_map, item_npcs, item_sitems, item_items, item_env_inter, q1);
    let q2 = if is_cave_o {
        3
    } else {
    ...
    //quads.retain(|&x| x != q2);
    let (q2_map, q2_npcs, q2_sitems, q2_items, q2_env_inter) = place_small_parts(q1_map.clone(), guild_map, guild_npcs, guild_sitems, guild_items, guild_env_inter, q2);
    let q3 = if is_cave_o {
        4
    } else {
    ...
    //quads.retain(|&x| x != q3);
    let (q3_map, q3_npcs, q3_sitems, q3_items, q3_env_inter) = place_small_parts(q2_map.clone(), church_map, church_npcs, church_sitems, church_items, church_env_inter, q3);
    let q4 = if is_cave_o {
        2
    } else {
    ...
    let (final_map, q4_npcs, q4_sitems, q4_items, q4_env_inter) = place_small_parts(q3_map.clone(), anchor_map, anchor_npcs, anchor_sitems, anchor_items, anchor_env_inter, q4);
    ...
    (final_map, final_npcs, final_sitems, final_items, final_env_inter)
}
...
impl Settlement {
    pub fn new(stype: Settle, sname: String, pos: (i64, i64), npcs: HashMap<(usize, usize), NPCWrap>, items: HashMap<(usize, usize), Item>, shops: HashMap<Shops, Shop>, env_inters: HashMap<(usize, usize), EnvInter>, map: Vec<Vec<Cells>>) -> Self {
        Self {stype, sname, pos, npcs, items, npcs_sent: false, items_sent: false, shops, env_inters, map}
    }
